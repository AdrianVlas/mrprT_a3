...
#define FIRST_INDEX_NUMBER_MAX_PHASE_DR       47//47
#define FIRST_INDEX_NUMBER_MAX_PHASE04_DR     48//48
#define FIRST_INDEX_NUMBER_MAX_3I0_DR         48//49
#define FIRST_INDEX_NUMBER_MAX_3U0_DR         48//50
#define FIRST_INDEX_NUMBER_MIN_U_DR           48//51
#define FIRST_INDEX_NUMBER_MAX_U_DR           48//52
#define FIRST_INDEX_NUMBER_MAX_ZOP_DR         48//53
#define FIRST_INDEX_NUMBER_MIN_F_ACHR_DR      48//54
#define FIRST_INDEX_NUMBER_F_CHAPV_DR         48//55
#define FIRST_INDEX_FIRST_BLOCK_DR            49//56
#define FIRST_INDEX_FIRST_DATA_DR             50//1176
...
30(с)х50(періодів)х16(виборок на період)х(12(канали) + 18(двобайтні слова дискретних сигналів))
х2(байти на число) = 1440000(байти)
Ще має бути 1(байт мітки початку запису )+7(байти мітки часу)+4(байти коеф.тр.ТС(ВН))+
4(байти коеф.тр.ТС(НН))+4(байти коеф.тр.ТН)+32(байти імені комірки) = 52 байти
...
52 є кратним 2 - отже, все нормально!

Тобто розмір максимального аналогового запису 1440000+52 = 1440052(байти)
З урахуванням того, що розмір сторінки DF становить 512 байт, то максимальний
аналоговий запис становить 1440052/512 = 2812,6015625 сторінки. Значить буде вестися 
запис у 2813 сторінки DF.
...
Тоді для запису 2813 сторінок треба буде
2251(сторінки)х36(мс) = 101 268(мс)=101,268(с)

Тобто процес запису максимального аналогового запису буде тривати 101,268(с)

Швидкість "заповнення" миттєвими виборками і дискретними сигналами з вимірювальної системи:
(12(канали) + 18(двобайтні слова дискретних сигналів))х2(байти на значення)/1,25(мс
- інтервал між виборками при частоті 50Гц і 16 виборок на період) = 48,0 (байт/мс)
...
2813(сторінки)х512(байт на сторінку)/101268(мс) = 14,(2) (байт/мс)
...
буде виконуватися зі швидкістю: 48,0 - 14,(2) = 33,78(байт/мс)
...
25000(мс)*33,78(байт/мс) = 844500(байт) <= 844500(байт)
844500(байт)/512(байт/стор.) = 1649,4140625(сторінок) < 1650(сторінок)

Крім того треба мати на увазі те, що у записі аналогового реєстратора є 52 байт
...
зберігання миттєвих значень мало б вистачити у (розмір доаварійного масису + 
(1651 + 1) сторінки DF). Ще одну сторінку до 1650 теоретично розрахованих я додав 
для перестаховки.

Тобто розмір має бути
5(с - макс. доавар. мас.)х50(пром.частота)х16(кільк. вибор.за пер. для ан.р.)х(12(канали) + 18(двобайтні слова дискретних сигналів)) +1651*[512(байт)/2(байт на одне миттєве значення)](кількість миттєвих значень на сторінці)=
= 120000 +1651х256 = 542656
...
1651(сторінок)х256(слів) = 422656 (слів)

422656/(12 + 18)= 14088,533333333333333333333333333
Якщо ділення не є ціле число, то треба провести корекцію, щоб Х/(12 + 18)
давало ціле число, причому Х >= 422656 - тобто беремо найслижче число X = 422670

таким чином
120000 + 422670 = 542670

!!!Вищевикладена дає мені підставу ширину масиву встановити 542670 елементи (двохбайтені,
